...
export class MailpitClient {
  ...
  private async request<T>(
    ...
    const { params, body, binary = false } = options;
    ...
    try {
      if (binary) {
        ...
        // In Node.js, convert to Buffer for compatibility; in browsers, keep as ArrayBuffer.
        data = (IS_NODE ? Buffer.from(ab) : ab) as T & (ArrayBuffer | Buffer);
      } else {
  ...
  public async getMessageSource(id: string = "latest"): Promise<string> {
    return await this.handleRequest<string>("GET", `/api/v1/message/${id}/raw`);
  }
